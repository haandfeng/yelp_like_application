...
public class PageDTO<T> {
    ...
    public static <T> PageDTO<T> of(long total, long pageSize, List<T> data) {
        long pages = pageSize <= 0 ? 0 : (long) Math.ceil((double) total / pageSize);
        return new PageDTO<>(total, pages, data);
    }
}
